...
class GeneralizedNoSQLAgent:
    def __init__(self, connection_string: str = "mongodb://localhost:27017/", database_name: str = "user_management_db"):
        """
...
5. For regex patterns:
   - Use double backslashes for escaping special characters
   - Example: {{"$regex": "\\\\.com$"}} for matching strings ending in .com
   - Example: {{"$regex": "^user_"}} for matching strings starting with user_
   - Example: {{"$regex": "\\\\d{3}-\\\\d{3}-\\\\d{4}"}} for matching phone numbers
...
7. Always use double backslashes for regex patterns
