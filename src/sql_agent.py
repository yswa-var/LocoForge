...
def main():
    ...
    try:
        ...
        result = agent.execute_query(f"Insert a new record for {today} with open=100, high=110, low=95, close=105, volume=1000")
        print(result)
        
        print("\n3. Updating the record:")
        result = agent.execute_query(f"Update the record for {today} to set volume to 2000")
        print(result)
        
        print("\n4. Deleting the record:")
        result = agent.execute_query(f"Delete the record for {today}")
        print(result)
